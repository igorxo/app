        ...
        while hash_slice:
            ...
            if status:
                ...
                for report in reports:
                    detections = 0
                    totalengines = len(enginescfg.engines)
                    if report['response_code'] != 1:
                        unknowns.append(report['resource'])
                        continue

                    for engine, detect in report['scans'].items():
                        if engine in enginescfg.engines and detect['detected']:
                            detections = detections + 1

                    if detections == 0:
                        clean.append((report['resource'], detections, totalengines))
                    elif detections / float(totalengines) >= self.BAD_RATIO:
                        bad.append((report['resource'], detections, totalengines))
                    elif detections / float(totalengines) >= self.HIGH_RATIO:
                        high.append((report['resource'], detections, totalengines))
                    else:
                        low.append((report['resource'], detections, totalengines))
